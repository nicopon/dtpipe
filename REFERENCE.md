...
| `--llm-timeout` | | Max silence between tokens before the turn ends with a stated error — a technical ceiling on the LLM, not a budget for the turn: a model that keeps producing output is never cut off, however long its answer. Identical on both paths, the blocking one reading the same stream with nobody watching it | `300` |
