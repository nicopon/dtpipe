...
public sealed partial class SqlServerStreamReader : IStreamReader, IRequiresOptions<SqlServerReaderOptions>
{
	...
	private static List<PipeColumnInfo> ExtractColumns(SqlDataReader reader)
	{
		...
		foreach (DataRow row in schemaTable.Rows)
		{
			...
			// SQL Server is generally case-insensitive (CI) by default, though CS collations exist.
			// For the purpose of the StreamReader, treating columns as case-insensitive is the safest default
			// to allow matching "ID" with "id" in downstream mapping.
