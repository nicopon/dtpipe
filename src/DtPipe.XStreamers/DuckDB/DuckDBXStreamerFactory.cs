...
namespace DtPipe.XStreamers.DuckDB;

public class DuckDBXStreamerFactory : IXStreamerFactory
{
    public string ComponentName => "duck-engine";
    public string Category => "XStreamers";
    public Type OptionsType => typeof(DuckDBXStreamerOptions);

    // DuckDBXStreamer consumes Arrow RecordBatch channels directly and pipes them to the engine
    ...
    {
        var opts = (DuckDBXStreamerOptions)options;
        var registry = serviceProvider.GetRequiredService<IMemoryChannelRegistry>();
        var logger = serviceProvider.GetRequiredService<ILoggerFactory>().CreateLogger<DuckDBXStreamer>();

        return new DuckDBXStreamer(
            registry,
...
public class DuckDBXStreamerOptions : IOptionSet, ICliOptionMetadata
{
