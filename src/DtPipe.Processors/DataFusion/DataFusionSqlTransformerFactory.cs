...
/// <summary>
/// Factory for <see cref="Sql.SqlStreamTransformer"/> backed by the DataFusion native engine.
/// Activated by <see cref="CompositeSqlTransformerFactory"/>. This is the default SQL engine.
/// Can also be explicitly requested via <c>--sql-engine datafusion</c> or <c>DTPIPE_SQL_ENGINE=datafusion</c>.
/// </summary>
public class DataFusionSqlTransformerFactory : IStreamTransformerFactory
{
    public string ComponentName => "datafusion-sql";
    ...
    public IStreamTransformer Create(string[] branchArgs, BranchChannelContext ctx, IServiceProvider serviceProvider)
    {
        var query = BranchArgParser.ExtractValue(branchArgs, "--sql")
            ?? throw new ArgumentException("--sql <query> is required for DataFusionSqlTransformerFactory");
