...
TEST_CSV="$ARTIFACTS_DIR/dtfusion_ref.csv"
...
"$DTFUSION_CMD" \
  ...
  --out "csv:$ARTIFACTS_DIR/dtfusion_out.csv"

# Step 5: Verify Output
echo "4. Verifying Output..."
cat "$ARTIFACTS_DIR/dtfusion_out.csv" | grep "Alice" > /dev/null
cat "$ARTIFACTS_DIR/dtfusion_out.csv" | grep "Bob" > /dev/null

if cat "$ARTIFACTS_DIR/dtfusion_out.csv" | grep "Eve"; then
    echo "❌ Error: Eve should not be in the output!"
    exit 1
fi

rm "$TEST_CSV"
rm "$ARTIFACTS_DIR/dtfusion_out.csv"
