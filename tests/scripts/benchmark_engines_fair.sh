...
MAIN_PROC="$DTPIPE_CMD --input parquet:$MAIN_PARQUET --output arrow --no-stats"
REF_PROC="$DTPIPE_CMD --input csv:$REF_CSV --output arrow --no-stats"
REF2_PROC="$DTPIPE_CMD --input csv:$REF2_CSV --output arrow --no-stats"
...
/usr/bin/time -l "$DTDUCK_CMD" \
  ...
  --query "$QUERY" --out "csv:$ARTIFACTS_DIR/temp_result.csv" 2>&1 | grep -v "^\s*0 " | head -20 || true
...
/usr/bin/time -l "$DTFUSION_CMD" \
  ...
  --in ref="csv:$REF_CSV" \
  --in ref2="csv:$REF2_CSV" \
  --query "$QUERY_FUSION" --out "csv:$ARTIFACTS_DIR/temp_result.csv" 2>&1 | grep -v "^\s*0 " | head -20 || true
...
/usr/bin/time -l "$DTDUCK_CMD" \
  ...
  --query "$QUERY" --out "csv:$ARTIFACTS_DIR/temp_result.csv" || true
...
/usr/bin/time -l "$DTFUSION_CMD" \
  ...
  --in "ref=proc:$REF_PROC" \
  --in "ref2=proc:$REF2_PROC" \
  --query "$QUERY_FUSION" --out "csv:$ARTIFACTS_DIR/temp_result.csv" || true
