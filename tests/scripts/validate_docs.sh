...
ALLOW_LIST="--project --version --help --fake-list --secrets --columnar-fast-path --compute-types --linux-pipes --migration --install --table --strategy --key --sampling-rate --sampling-seed --export-job --auto-migrate --strict-schema --metrics-path --unsafe-query --insert-mode --sql-processors --no-schema-validation --pre-exec --post-exec --finally-exec --from --ref --alias --src-main --src-ref --max-retries --retry-delay-ms"
FAILED=0
