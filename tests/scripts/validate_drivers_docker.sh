...
docker run --name dtpipe-postgres -e POSTGRES_PASSWORD=mysecretpassword -p 5432:5432 -d postgres:latest
# Wait for PG to be ready
echo "Waiting for Postgres..."
sleep 5

# Create seed data
docker exec -i dtpipe-postgres psql -U postgres <<EOF
...
docker run --name dtpipe-mssql -e "ACCEPT_EULA=Y" -e "MSSQL_SA_PASSWORD=MySecretPassword123!" -p 1433:1433 -d mcr.microsoft.com/azure-sql-edge:latest
...
docker run --rm --network container:dtpipe-mssql -i mcr.microsoft.com/mssql-tools /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P "MySecretPassword123!" <<EOF
...
docker run --name dtpipe-oracle -e ORACLE_PASSWORD=MySecretPassword123! -p 1521:1521 -d gvenzl/oracle-free:slim
...
docker exec -i dtpipe-oracle sqlplus system/MySecretPassword123!@localhost:1521/FREEPDB1 <<EOF
